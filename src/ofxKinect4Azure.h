...
protected:
	...
	OFX_K4A_COLORIZE_DEPTH_TYPE  colorize_mode = DEPTH_MASK; //DEPTH_MASK /BLUE_TO_RED 
	bool	bTablesCreated = false;
	ofColor depthMaskTable[USHRT_MAX];
	ofColor blueRedTable[USHRT_MAX];
	ofColor grayscaleTable[USHRT_MAX];
...
public:
	...
	glm::vec2 getDepthModeRange(const k4a_depth_mode_t depth_mode)
	{
		...
		case K4A_DEPTH_MODE_NFOV_2X2BINNED:
			return range = glm::vec2(0, 500); //500, 5800, 250, 1000
		case K4A_DEPTH_MODE_NFOV_UNBINNED:
			return range = glm::vec2(0, 500); //500, 4000 250, 1000
		case K4A_DEPTH_MODE_WFOV_2X2BINNED:
			return range = glm::vec2(300, 335); // 250, 3000
		case K4A_DEPTH_MODE_WFOV_UNBINNED:
			return range = glm::vec2(250, 300);//250, 2500
	...
	static inline ofColor DepthMask(const unsigned short &depthPixel,
		float &min,
		float &max)
	{
		constexpr unsigned char PixelMax = std::numeric_limits<unsigned char>::max();
		// Default to opaque black.
		//
		ofColor result = ofColor(255, 255, 255, PixelMax);
		// If the pixel is actual zero and not just below the min value, make it black
		//
		if (depthPixel == 0)
		{
		...
		// Normalize to [0, 1]
		//
		float lightness = (clampedValue - min) / (max - min);
		//lightness = 1 - lightness;
		if (lightness < 1) result = ofColor(0, 0, 0, PixelMax);
		else result = ofColor(PixelMax * lightness, PixelMax * lightness, PixelMax * lightness, PixelMax);
		return result;
	}

	static inline ofColor ColorizeGrayscale(const unsigned short &depthPixel,
		float &min,
		float &max)
	{
		constexpr unsigned char PixelMax = std::numeric_limits<unsigned char>::max();
		...
		float lightness = (clampedValue - min) / (max - min);
		lightness = 1 - lightness;
		result = ofColor(PixelMax * lightness, PixelMax * lightness, PixelMax * lightness, PixelMax);
		return result;
	}
