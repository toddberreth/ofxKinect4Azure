...
void ofxKinect4Azure::setup(ofxKinect4AzureSettings _settings)
{
	...
	if (!bTablesCreated) {
		...
		for (int x = 0; x < USHRT_MAX; x++) {
			depthMaskTable[x] = DepthMask(x, range.x, range.y);
			grayscaleTable[x] = ColorizeGrayscale(x, range.x, range.y);
			blueRedTable[x] = ColorizeBlueToRed(x, range.x, range.y);
		}
	}
...
void ofxKinect4Azure::setup(int index, ofxKinect4AzureSettings _settings)
{
	...
	if (index >= 0 && device_count > index)
	{
		...
		if (settings.make_colorize_depth) colorized_depth_pix.allocate(depth_size.x, depth_size.y, OF_PIXELS_BGRA);
	}
	else
	{
...
//			ofLogError("ofxKinct4Azure") << "capture failed.";
		...
		if (settings.depth_mode != K4A_DEPTH_MODE_OFF)
		{
			...
			if (settings.transform_type == DEPTH_TO_COLOR)
			{
				...
				transformed_depth_image = k4a::image::create(K4A_IMAGE_FORMAT_DEPTH16, color_image.get_width_pixels(),color_image.get_height_pixels(), color_image.get_width_pixels() * static_cast<int32_t>(sizeof(uint16_t)));
			...
			if (settings.make_colorize_depth) {
				if (!colorized_depth_pix.isAllocated()) colorized_depth_pix.allocate(depth_size.x, depth_size.y, OF_PIXELS_BGRA);
				...
				for (int h = 0; h < depth_size.y; ++h)
				{
					for (int w = 0; w < depth_size.x; ++w)
					{
						...
						ofColor colorized;
						//if (colorize_mode == BLUE_TO_RED) colorized = ColorizeBlueToRed(d_data[current_pixel], range.x, range.y);
						//if (colorize_mode == GRAYSCALE) colorized = ColorizeGreyscale(d_data[current_pixel], range.x, range.y);
						//if (colorize_mode == DEPTH_MASK) colorized = DepthMask(d_data[current_pixel], range.x, range.y);
						if (colorize_mode == BLUE_TO_RED) colorized = blueRedTable[d_data[current_pixel]];
						if (colorize_mode == GRAYSCALE) colorized = grayscaleTable[d_data[current_pixel]];
						if (colorize_mode == DEPTH_MASK) colorized = depthMaskTable[d_data[current_pixel]];
